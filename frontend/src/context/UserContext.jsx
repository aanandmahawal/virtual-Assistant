...
function UserContext({ children }) {
  const serverUrl = "https://virtual-assistant-backend-c10w.onrender.com";
...
const getGeminiResponse = async (command) => {
  try {
    if (!userData?.assistantName || !userData?.name) {
      console.warn("Assistant or user name missing");
      return { response: "Assistant name or user name is missing." };
    }
    ...
    return {
      type: "general",
      userInput: command,
      response: "Something went wrong while contacting the assistant."
    };
  }
};
